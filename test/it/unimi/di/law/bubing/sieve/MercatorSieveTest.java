...
public class MercatorSieveTest {	
	...
	public void testMultiThreadedTestSequentialDequeue() throws InterruptedException, IOException {
		...
		final AbstractSieve.DiskNewFlow<CharSequence> newFlow = new AbstractSieve.DiskNewFlow<CharSequence>( CharSequenceByteSerializerDeserializer.getInstance() );
		final File tempDir = Files.createTempDir();
		System.err.println( "testMultiThreadedTestSequentialDequeue() store in " + tempDir );
		final AbstractSieve<CharSequence,Integer> sieve = new MercatorSieve<CharSequence,Integer>( true, tempDir, NUM_THREADS * 100 * NUM_PAIRS, 512, NUM_THREADS * 100 * NUM_PAIRS, newFlow, CharSequenceByteSerializerDeserializer.getInstance(), ByteSerializerDeserializer.INTEGER, MercatorSieve.CHAR_SEQUENCE_HASHING_STRATEGY, null );
		...
		for ( int i = 0; i < NUM_PAIRS; i++ ) {
			...
			pairsToBeInserted[ i ] = new AbstractSieve.SieveEntry<CharSequence,Integer>( key, value );
		...
		for ( int i = 0; i < NUM_THREADS; i++ )
			insertionThread[ i ] = new Thread() {
				...
				public void run() {
					try {
						int times = 10 + random.nextInt( 90 );
						for ( int i = 0; i < times; i++ ) {
							sleep( random.nextInt( 40 ) );
							synchronized ( pairsToBeInserted ) {
								int pos = random.nextInt( NUM_PAIRS );
	...
	public void testMultiThreadedTestParallelDequeue() throws InterruptedException, IOException {
		...
		final AbstractSieve<CharSequence,Integer> sieve = new MercatorSieve<CharSequence,Integer>( true, tempDir, 1024, 512, 1024, newFlow, CharSequenceByteSerializerDeserializer.getInstance(), ByteSerializerDeserializer.INTEGER, MercatorSieve.CHAR_SEQUENCE_HASHING_STRATEGY, null );
		...
		final ObjectOpenHashSet<MutableString> enqueued = new ObjectOpenHashSet<MutableString>(); // Elements that were inserted
		...
		// Dequeuing threads
		final ObjectOpenHashSet<MutableString> dequeued = new ObjectOpenHashSet<MutableString>(); // Elements that were inserted
	...
	public void testSimple() throws IOException, InterruptedException {
		...
		AbstractSieve<CharSequence,Integer> sieve = new MercatorSieve<CharSequence,Integer>( true, tempDir, 1024, 16, 1024, newFlow, CharSequenceByteSerializerDeserializer.getInstance(), ByteSerializerDeserializer.INTEGER, MercatorSieve.CHAR_SEQUENCE_HASHING_STRATEGY, null );
