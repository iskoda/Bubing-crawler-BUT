...
public class HTMLParser<T> implements Parser<T> {
	...
	public HTMLParser<T> clone() {
		return new HTMLParser<T>( digestAppendable == null ? null : digestAppendable.hashFunction, textProcessor == null ? null : textProcessor.copy(), crossAuthorityDuplicates, buffer.length );
	}
	...
	public static void main( String arg[] ) throws IllegalArgumentException, IOException, URISyntaxException, JSAPException, NoSuchAlgorithmException {
		...
		final HTMLParser<Void> htmlParser =  new HTMLParser<Void>( BinaryParser.forName( digester ), (TextProcessor<Void>)null, crossAuthorityDuplicates, charBufferSize );
