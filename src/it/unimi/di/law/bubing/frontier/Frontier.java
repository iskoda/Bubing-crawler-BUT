...
public class Frontier implements JobListener<BubingJob>, AbstractSieve.NewFlowReceiver<ByteArrayList> {
	...
	public Frontier( final RuntimeConfiguration rc, final Store store, final Agent agent ) throws IOException, IllegalArgumentException, ConfigurationException, ClassNotFoundException,
		...
		if ( rc.sieveSize == 0 ) sieve = new IdentitySieve<ByteArrayList, Void>( this, new ByteArrayListByteSerializerDeserializer(), ByteSerializerDeserializer.VOID,
				BYTE_ARRAY_LIST_HASHING_STRATEGY, null );
		else sieve = new MercatorSieve<ByteArrayList, Void>( rc.crawlIsNew, rc.sieveDir, rc.sieveSize, rc.sieveStoreIOBufferByteSize, rc.sieveAuxFileIOBufferByteSize, this,
				new ByteArrayListByteSerializerDeserializer(), ByteSerializerDeserializer.VOID, BYTE_ARRAY_LIST_HASHING_STRATEGY, null );

		this.agent = agent;
		this.workbench = new Workbench();
		this.unknownHosts = new DelayQueue<VisitState>();
		...
		dnsThreads = new ObjectArrayList<DNSThread>();
		fetchingThreads = new ObjectArrayList<FetchingThread>();
		parsingThreads = new ObjectArrayList<ParsingThread>();
		newVisitStates = new LinkedBlockingQueue<VisitState>();
		...
		quickReceivedURLs = new ArrayBlockingQueue<ByteArrayList>( 1024 );
